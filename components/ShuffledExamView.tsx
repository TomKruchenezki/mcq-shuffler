  ...
  return (
    <section className="my-6 space-y-4" dir="rtl">
      ...
      {exam.questions.map(q => (
        <article key={q.outputQuestionNumber} className="bg-white rounded-xl border border-gray-200 p-5 space-y-3">
          <p dir="rtl" className="font-semibold text-gray-800 leading-relaxed">
            <strong className="text-blue-600 ml-2">שאלה {q.outputQuestionNumber}:</strong>
            {q.number !== q.outputQuestionNumber && (
              <span className="text-xs text-gray-400 mr-1">(מקור: {q.number})</span>
            )}
